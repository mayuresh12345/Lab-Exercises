...
int main(void)
{
    ...
    fp = fopen("/Users/tanmaygulati/Work/3rd Year/PCAP\ OpenCL/Lab6/Q1/Q1/multipleStrings.cl", "r");
    ...
    cl_mem t_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, N * (len + 1) * sizeof(char), NULL, &ret);
    ...
    cl_kernel kernel = clCreateKernel(program, "multipleStrings", &ret);
    ...
    char *strres = (char *)malloc(sizeof(char) * N * (len+1));
    ret = clEnqueueReadBuffer(command_queue, t_mem_obj, CL_TRUE, 0, N * (len + 1) * sizeof(char), strres, 0, NULL, NULL);
    printf("Done\n");
    strres[N * (len+1)] = '\0';
