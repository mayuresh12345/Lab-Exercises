...
//    scanf("%s", tempstr);
    ...
    int *indices = (int *)malloc(100 * sizeof(int));
    
    int k = 1;
    indices[0] = -1; // for first word
    for(i = 0; i < len; i++)
    {
        if(str[i] == ' ')
        {
...
//            printf("%d\t", i);
    ...
    fp = fopen("/Users/tanmaygulati/Work/3rd Year/PCAP\ OpenCL/Lab6/Q4/Q4/reverse_word.cl", "r");
    ...
    cl_mem indices_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, k * sizeof(int), NULL, &ret);
    ...
    ret = clEnqueueWriteBuffer(command_queue, indices_mem_obj, CL_TRUE, 0, k * sizeof(int), indices, 0, NULL, NULL);
    ...
    cl_kernel kernel = clCreateKernel(program, "reverse_word", &ret);
    ...
    ret = clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *)&indices_mem_obj);
    ...
    ret = clEnqueueReadBuffer(command_queue, t_mem_obj, CL_TRUE, 0, (len+1) * sizeof(char), strres, 0, NULL, NULL);
    printf("Done\n");
    strres[len+1] = '\0';
