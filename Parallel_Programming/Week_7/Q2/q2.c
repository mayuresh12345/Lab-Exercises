...
int main(){
	...
	cl_mem b_mem_obj = clCreateBuffer(context,CL_MEM_WRITE_ONLY,size_A*sizeof(int),NULL,&ret);
	...
	//Create kernel
	cl_kernel kernel = clCreateKernel(program,"mat_ones_comp",&ret);

	//Set Kernel Args
	int widthA= N;
	ret = clSetKernelArg(kernel,0,sizeof(cl_mem),(void *)&a_mem_obj);
	ret = clSetKernelArg(kernel,1,sizeof(cl_mem),(void *)&b_mem_obj);
	ret = clSetKernelArg(kernel,2,sizeof(cl_int),(void *)&widthA);
	...
	ret = clEnqueueReadBuffer(command_queue,b_mem_obj,CL_TRUE,0,sizeof(int)*size_A,b,0,NULL,NULL);
	printf("Matrix B is: \n");
	for(int i = 0; i < size_A; i++){
		printf("%u\t",b[i]);
		if((i+1)%N == 0)
